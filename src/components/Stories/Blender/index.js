...
const NAMING_PLAYLIST = "NAMING_PLAYLIST";
...
const BlenderModal = ({ generatePlaylist, playlistStatus, setPlaylistStatus, toggleModal, playlistLink }) => {
    ...
    return (
        <div className="Blender-Modal">
            ...
            { playlistStatus == NAMING_PLAYLIST && 
                <>
                    <h1>name your playlist</h1>
                    <input type="text" value={playlistName} onChange={(e) => setPlaylistName(e.target.value)}></input>
                    { playlistName.length > 0 && <Button onClick={handleButtonClick} width={200} >create</Button>}
                </>
            }
...
const Blender = ({ user_1, user_2, my_id, setSwipeDisable }) => {
    ...
    const [playlistStatus, setPlaylistStatus] = useState(NAMING_PLAYLIST)
    ...
    const toggleModal = () => {
        ...
        if (!showModal) {
            setPlaylistStatus(NAMING_PLAYLIST);
        }
    }
